...
export default function Home() {
  ...
  if (percentage >= 1) {
    percentage = (lineCleared - curLv) / (lineRequiredAtLevel(level+1) - curLv);
  }
  ...
  return (
    <main className={`grid grid-cols-[auto_200px_400px_200px_auto] ${luckiest_guy.className}`}>
      ...
      <div className="flex flex-col justify-center gap-20 items-center col-start-2">
        ...
        />
        <div className="h-1" >
          {isPlaying ? null : <button className="text-2xl text-center bg-white text-black p-2" onClick={startGame}>New Game</button>}
        </div>
      </div>
      <div>
        <h1 className={`text-3xl tracking-wider mt-[12px] text-center p-4 ${luckiest_guy.className}`}>TETRIS <span className="bg-white text-black pl-1 pr-1">BLACK</span> & WHITE</h1>
        <Board board={board} />
      </div>
